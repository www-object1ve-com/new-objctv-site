...
if ( ! class_exists( 'ACF_Internal_Post_Type' ) ) {
	class ACF_Internal_Post_Type {
		...
		public function __construct() {
			...
			add_action( 'acf/validate_field_group', array( $this, 'translate_post' ) );
			add_action( 'acf/validate_post_type', array( $this, 'translate_post' ) );
			add_action( 'acf/validate_taxonomy', array( $this, 'translate_post' ) );
		...
		public function get_raw_post( $id = 0 ) {
			...
			$raw_post = (array) maybe_unserialize( $post->post_content );
		...
		public function filter_posts( $posts, $args = array() ) {
			...
			if ( $args && is_array( $args ) && ! empty( $args['active'] ) && $args['active'] ) {
				$posts = array_filter(
					$posts,
					function( $post ) {
						return $post['active'];
